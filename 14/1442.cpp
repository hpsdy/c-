...
int main(int argc, char const *argv[]) {
    ...
    std::for_each(vec.begin(), vec.end(), std::bind(std::plus<std::string>(), "pref", std::placeholders::_2));
    std::cout << "========" << std::endl;
    for (auto &c : vec) {
        std::cout << c << std::endl;
    }
    std::vector<int> intarr{2, 4, 6, 8, 10};
    num = std::count_if(vec.cbegin(), vec.cend(), std::bind(std::modulus<int>(), 10, std::placeholders::_2));
    std::cout << num << std::endl;
    input(std::istream & io, std::vector<std::string> &vec) *p = std::bind(input);
